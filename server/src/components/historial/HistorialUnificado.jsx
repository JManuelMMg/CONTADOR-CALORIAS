...
const HistorialUnificado = () => {
  ...
  const [filtro, setFiltro] = useState('todos'); // 'todos', 'estacionamiento', 'autolavado'
  ...
  const totales = historialFiltrado.reduce((acc, item) => {
    acc.total += item.costo;
    if (item.tipo === 'estacionamiento') {
      acc.estacionamiento += item.costo;
    } else {
      acc.autolavado += item.costo;
    }
    return acc;
  ...
  const generarFactura = (item) => {
    const factura = `
      ...
      Placa: ${item.placa}
      ${item.tipo === 'estacionamiento' ? `Entrada: ${formatearFecha(item.horaEntrada)}
      Salida: ${formatearFecha(item.horaSalida)}` : `Servicio: ${item.nombre}
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="bg-white rounded-lg shadow-lg p-6 glass">
        <h2 className="text-2xl font-bold mb-6 text-gradient">Historial de Servicios</h2>
        ...
        <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-blue-50 p-4 rounded-lg">
            ...
            <p className="text-2xl font-bold text-blue-600">${totales.total}</p>
          </div>
          <div className="bg-green-50 p-4 rounded-lg">
            <h3 className="text-lg font-semibold mb-2">Total Estacionamiento</h3>
            <p className="text-2xl font-bold text-green-600">${totales.estacionamiento}</p>
          </div>
          <div className="bg-purple-50 p-4 rounded-lg">
            <h3 className="text-lg font-semibold mb-2">Total Autolavado</h3>
            <p className="text-2xl font-bold text-purple-600">${totales.autolavado}</p>
          </div>
        </div>
            ...
            <tbody>
              {historialFiltrado.map((item, index) => (
                <tr key={index} className="border-t hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2 font-bold">${item.costo}</td>
                  <td className="px-4 py-2">
                    <button
